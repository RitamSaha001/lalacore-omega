...
Future<void> main() async {
  ...
  const int aiQuestionCount = 5;
  const bool aiPreferPyq = true;
  const int hardnessBoost = 0;
  const bool forceAiEngineFallback = false;

  const List<String> selectedChapters = <String>['Coordinate Geometry'];
  ...
  final Map<String, dynamic> basePayload = <String, dynamic>{
    'title': 'AI Practice Quiz • Coordinate Geometry (PYQ)',
    ...
    'search_hard_pyq': false,
    'pyq_mode': aiPreferPyq ? 'offline_first' : 'hybrid',
    'pyq_web_only_mode': false,
    'pyq_answer_retrieval_required': false,
    'fallback_ultra_hard_required': false,
    'require_type_variety': true,
    'chapter_coverage_required': true,
    'web_scope_subject': selectedSubject,
    'web_scope_chapters': selectedChapters,
    'web_scope_subtopics': subtopics,
    'chapter_signal_lock': const <String, List<String>>{
      'Coordinate Geometry': <String>[
        'coordinate geometry',
        'straight line',
        'pair of lines',
        'circle',
  ...
  Map<String, dynamic> bestRejectedResponse = <String, dynamic>{};
  List<ParsedQuestion> bestRejectedQuestions = <ParsedQuestion>[];
  ...
  for (int attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    _log(
      'Attempt ${attempt + 1}/$maxAttempts: requesting pipeline '
      '(difficulty=$tunedDifficulty, trap=${payload['trap_intensity']}, forceFallback=$forceAiEngineFallback)',
    );
    ...
    try {
      if (forceAiEngineFallback) {
        _log('Attempt ${attempt + 1}: forcing ai_engine_fallback path.');
        ...
        source = 'ai_engine_fallback_forced';
        _log(
          'Attempt ${attempt + 1}: forced fallback returned rawQuestions=${rawQuestions.length}',
        );
      } else {
        ...
        if (rawQuestions.isEmpty) {
          final String encoded = jsonEncode(generated);
          _log(
            'Attempt ${attempt + 1}: backend raw response snippet='
            '${encoded.substring(0, min(encoded.length, 3500))}',
          );
          if (!backendOk) {
      ...
      if (tooEasy) {
        if (parsed.length > bestRejectedQuestions.length ||
            bestRejectedQuestions.isEmpty) {
          bestRejectedQuestions = parsed;
          bestRejectedResponse = generated;
          bestRejectedReason = 'set too easy after validation';
        }
      ...
      if (!contract) {
        if (parsed.length > bestRejectedQuestions.length ||
          ...
          bestRejectedQuestions = parsed;
          bestRejectedResponse = generated;
          bestRejectedReason = 'custom practice contract failed';
        }
      ...
      try {
        ...
        if (tooEasy) {
          if (parsed.length > bestRejectedQuestions.length ||
            ...
            bestRejectedQuestions = parsed;
            bestRejectedResponse = generated;
            bestRejectedReason = 'fallback set too easy after validation';
          }
          failures.add(
        ...
        if (!contract) {
          if (parsed.length > bestRejectedQuestions.length ||
            ...
            bestRejectedQuestions = parsed;
            bestRejectedResponse = generated;
            bestRejectedReason = 'fallback custom practice contract failed';
          }
          failures.add(
  ...
  print('');
  print(
    '========== STUDENT AI GENERATION REPORT (COORDINATE GEOMETRY • PYQ MODE) ==========',
  );
  print('run_time: ${DateTime.now().toIso8601String()}');
  print('requested_chapter: Coordinate Geometry');
  ...
  print('web_scope_subject: $selectedSubject');
  print('web_scope_chapters: ${selectedChapters.join(", ")}');
  print('web_scope_subtopics: ${subtopics.join(", ")}');
  ...
  if (finalQuestions.isEmpty) {
    ...
    if (bestRejectedQuestions.isNotEmpty) {
      print('best_rejected_reason: $bestRejectedReason');
      print('best_rejected_question_count: ${bestRejectedQuestions.length}');
      print(
        'best_rejected_likely_too_easy: ${_quizLikelyTooEasy(bestRejectedQuestions)}',
      );
      print('');
      print('BEST_REJECTED_QUESTIONS:');
      for (int i = 0; i < bestRejectedQuestions.length; i++) {
        final ParsedQuestion q = bestRejectedQuestions[i];
        print('--- Q${i + 1} ---');
        print('type: ${q.type}');
      ...
      print('');
      print('BEST_REJECTED_RAW_RESPONSE_HEAD:');
      final String rejectedRaw = const JsonEncoder.withIndent(
        '  ',
      ).convert(bestRejectedResponse);
      print(rejectedRaw.substring(0, min(rejectedRaw.length, 5000)));
      return;
    }
    final String encoded = jsonEncode(finalResponse);
    print(
      'final_response_snippet=${encoded.substring(0, min(1200, encoded.length))}',
    );
    return;
  }
